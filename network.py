...
class GALPRUN():
    ...
    def train_linear(self,epoch_time, lr=0.001,momentum=0.9, weight_decay=5e-4,distance_rate=0.1,distance_rate_add=0.01,distance_rate_time=4):
        self.optimizer = optim.SGD(self.vggnet.parameters(), lr=lr,momentum=momentum, weight_decay=weight_decay)
        i_dis_t=0
        for epoch in range(epoch_time):
            st=time.time()
            ...
            if(i_dis_t>=distance_rate_time):
                ...
                i_dis_t=0
            print("Training Finished, TotalEPOCH=%d,Epochtime=%d" % (epoch,ed-st))
    def train_conv(self,epoch_time, lr=0.001,momentum=0.9, weight_decay=5e-4,distance_rate=0.1,distance_rate_add=0.01,distance_rate_time=4):
        self.optimizer = optim.SGD(self.vggnet.parameters(), lr=lr,momentum=momentum, weight_decay=weight_decay)
        i_dis_t=0
        for epoch in range(epoch_time):
            st=time.time()
